<script lang="ts">
  import { categories, getLogos } from '$lib/data/integrations';
</script>
      ...
      <div
        ...
        <div class="flex items-center gap-10">
          {#each getLogos(category) as item}
            <img class="h-8 grayscale hover:grayscale-0 transition-colors duration-150" src={item.src} alt={item.alt} loading="lazy" decoding="async" />
          {/each}
        </div>
        <div class="flex items-center gap-10" aria-hidden="true">
          {#each getLogos(category) as item}
            <img class="h-8 grayscale hover:grayscale-0 transition-colors duration-150" src={item.src} alt="" loading="lazy" decoding="async" />
          {/each}
        </div>
      </div>
    </div>
